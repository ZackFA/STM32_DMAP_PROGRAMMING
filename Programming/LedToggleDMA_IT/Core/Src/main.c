...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  HAL_DMA_Start_IT(&hdma_memtomem_dma2_stream0, (uint32_t )&led_data[0],(uint32_t) &GPIOA->ODR,(uint32_t) 1);

	  HAL_Delay(1000);

	  HAL_DMA_Start_IT(&hdma_memtomem_dma2_stream0, (uint32_t )&led_data[1],(uint32_t) &GPIOA->ODR,(uint32_t) 1);
